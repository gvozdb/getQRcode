...
$response = $this->modx->runProcessor('item/generate', array(
		...
		array('processors_path' => MODX_CORE_PATH.'components/qrcode/processors/mgr/')
	);
...
class QRcodeItemGenerateProcessor extends modObjectProcessor
{
	public $objectType = 'QRcodeItem';
	public $classKey = 'QRcodeItem';
	//public $languageTopics = array('qrcode');
	...
	public function initialize()
	{
		...
		$assetsUrl = $this->modx->getOption('qrcode_assets_url', null, $this->modx->getOption('assets_url') . 'components/qrcode/');
	...
	public function process()
	{
		...
		$path			= str_replace( $slashes, '/', $this->config['qrcodesPath'] .'/'. $params['type'] .'/' );
		$pathUrl		= str_replace( $slashes, '/', $this->config['qrcodesUrl'] .'/'. $params['type'] .'/' );
		...
		if( $insert )
		{
			$qrcodeObj = $this->modx->newObject( $this->classKey, array(
...
return 'QRcodeItemGenerateProcessor';